...
!!! note "Alignment with the paper"
    The strategy implements Eq. (12) from Wang et al. (NeurIPS 2020) in the following ways: average normalized updates with weight p_i τ_eff / τ_i, after first accumulating τ_eff = Σ_i p_i τ_i. This matches the description in §5/fn.11-12 of the paper.
...
!!! note "Alignment with the paper"
    - The projection head with L2-normalised embeddings follows Eq. (3) of Li et al. (CVPR 2021); see `examples/server_aggregation/moon/moon_model.py:31-79`.
    - The joint objective in `examples/server_aggregation/moon/moon_trainer.py:26-152` implements Eq. (1) by combining cross-entropy with a temperature-scaled contrastive loss against the global model (positive) and stored local models (negatives).
    - Client-side buffering of frozen checkpoints (`examples/server_aggregation/moon/moon_client.py:21-64`) and the server’s weighted averaging (`examples/server_aggregation/moon/moon_server_strategy.py:19-63`) mirror the official PyTorch implementation’s workflow while adopting Plato’s strategy interfaces for composition.
...
!!! note "Alignment with the paper"
    The Plato strategy keeps the original pipeline in the paper: it stacks floating-point client deltas, applies layer- wise PCA, seeds the estimator with the per-channel median, and feeds query/key pairs through a multi-pass attention block with temperature scaling and ε-thresholding before normalising weights, exactly mirroring Algorithm 1 and the PCA discussion in Sections 3.2–4.1 of Wan & Chen (2021). Refer to `attack_adaptive_server_strategy.py:425-486` and the shared attention module in `attack_adaptive_server_strategy.py:174-289`.
