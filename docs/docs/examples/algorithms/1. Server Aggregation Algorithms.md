...
!!! note "Alignment with the paper"
    ...
    - Local training objective – `moon_trainer.py:26-152` combines the supervised cross-entropy with the temperature-scaled contrastive loss exactly like Eq. (1): positives come from the frozen global model, negatives from the stored local- history models, using the same μ and τ hyper-parameters exposed in the config (`moon_MNIST_lenet5.yml:41-45`). This mirrors train_net_fedcon in the reference implementation, which also weights the contrastive term by μ and uses CrossEntropy on logits built from cosine similarities.
