...
Attack-Adaptive Aggregation is a robust server aggregation algorithm designed to defend against malicious clients in federated learning. Compared to FedAtt, this algorithm uses cosine similarity between client and server parameters instead of norm distance, and applies softmax with temperature scaling for computing attention weights. The implementation in `examples/server_aggregation/attack_adaptive/attack_adaptive_algorithm.py` overrides the default `aggregate_weights()` method to implement attack-adaptive aggregation as a custom server aggregation algorithm.

The algorithm works by calculating the cosine similarity between the baseline weights and each client's weight deltas, then applying a temperature-scaled softmax to compute attention weights for each client. These attention weights are used to perform a weighted aggregation of client updates, giving more weight to clients whose updates are more aligned with the server model.
...
**Reference:** C. P. Wan, Q. Chen. "[Robust Federated Learning with Attack-Adaptive Aggregation](https://arxiv.org/pdf/2102.05257.pdf)," 2021.
