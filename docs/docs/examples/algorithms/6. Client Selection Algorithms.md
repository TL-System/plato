...
In Plato's strategy-based implementation, Polaris combines a custom client-selection strategy with a custom aggregation strategy. The aggregation side extends FedAvg through `aggregate_deltas()` so the server can keep tracking gradient bounds for unexplored clients while still using the standard delta-based aggregation flow.
