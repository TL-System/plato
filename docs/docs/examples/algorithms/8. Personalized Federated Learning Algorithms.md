...
!!! note "Alignment with the paper"
    The FedBABU callback (`examples/personalized_fl/fedbabu/fedbabu_trainer.py:15-63`) freezes local layers during collaborative rounds and flips to freezing the global body once personalization starts, reproducing the two-phase training schedule described by Oh et al.

    Layer freezing and reactivation reuse the shared utilities in `plato.utils.trainer_utils`, so only representation parameters move to the server while client-specific heads stay untouched, matching the original workflow.
...
!!! note "Alignment with the paper"
    `plato/trainers/strategies/algorithms/personalized_fl_strategy.py:1-176` freezes the shared layers whenever `current_round` exceeds the global training budget, so only the personalization head keeps training, which matches the procedure in Arivazhagan et al.

    The same strategy keeps all layers trainable during collaborative rounds, ensuring FedPer's global backbone is learned jointly before clients specialize their local heads.
...
!!! note "Alignment with the paper"
    The Per-FedAvg training step (`examples/personalized_fl/perfedavg/perfedavg_trainer.py:18-135`) performs the two gradient updates with fixed alpha and beta learning rates, restores the pre-update weights, and then applies the meta-gradient, mirroring Algorithm 1 from Fallah et al.

    When personalization rounds begin, the strategy bypasses the meta-step and switches to standard SGD, matching the paper's fine-tuning phase for each client.
