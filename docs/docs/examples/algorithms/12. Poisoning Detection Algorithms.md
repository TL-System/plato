...
Datasets are downloaded automatically using the paths defined in each configuration file, so no additional flags are required. Modify these YAML files to explore custom scenarios or to adjust hyperparameters for new experiments.
