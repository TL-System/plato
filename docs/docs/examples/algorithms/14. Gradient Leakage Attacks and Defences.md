...
Run the program using the following command under the directory `examples/gradient_leakage_attacks`. The following example launches the csDLG attack using delta matching against an untrained ResNet18 model in `.eval()` mode on CIFAR100 data:

```bash
cd examples/gradient_leakage_attacks
uv run dlg.py -c untrained_eval_delta.toml
```

## Run Standard Federated Learning Training

Prepare a pre-trained model for DLG attack evaluation by running one of the examples below:

```bash
# ResNet-18 on CIFAR-100
uv run fedavg_pretraining.py -c fedavg_resnet18_cifar100.toml

# ResNet-152 on CIFAR-100
uv run fedavg_pretraining.py -c fedavg_resnet152_cifar100.toml
```

## DLG Related Configurations

Try tuning the following hyperparameters in `.toml` configuration files.
