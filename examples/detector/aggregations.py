...
def krum(updates,baseline_weights, weights_attacked):
        ...
        for deltas_vector in remaining_weights:
            distance = (
                torch.norm((remaining_weights - deltas_vector), dim=1) ** 2
            )
            distances = (
                distance[None, :]
                if not len(distances)
...
def multi_krum(updates, baseline_weights, weights_attacked):
        ...
        while len(remaining_deltas_vector) > 2 * num_attackers_selected + 2:
            ...
            scores = torch.sum(
                distances[:, : len(remaining_deltas_vector) - 2 - num_attackers_selected], dim=1
            )
...
def fl_trust(updates, baseline, weights_attacked):
        ...
        all_deltas_vector = flatten_weights(weights_attacked)
        num_clients,d = all_deltas_vector.shape

        model_re = torch.mean(all_deltas_vector, dim=0).squeeze()
        cos_sims = []
        candidates = []

        # compute cos similarity
        for delta_vector in all_deltas_vector:
            cos_sim = (
                torch.dot(delta_vector.squeeze(), model_re)
        ...
        for i in range(num_clients):
            candidate = (
                ...
                * normalized_weights[i]
                / torch.norm(all_deltas_vector[i] + 1e-9)
                * torch.norm(model_re)
            )
            candidates = (
        ...
        mean_delta_vector = torch.sum(candidates, dim=0)
