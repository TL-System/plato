    ...
    async def federated_averaging(self, updates):
        ...
        for i, update in enumerate(weights_received):
            ...
            (
                update_from_actor,
                update_from_critic,
                update_from_actor_target,
                update_from_critic_target,
            ) = update

            for name, delta in update_from_actor.items():
                actor_avg_update[name] += delta * (num_samples / self.total_samples)

            for name, delta in update_from_critic.items():
                critic_avg_update[name] += delta * (num_samples / self.total_samples)

            for name, delta in update_from_actor_target.items():
                actor_target_avg_update[name] += delta * (
                    num_samples / self.total_samples
                )

            for name, delta in update_from_critic_target.items():
                critic_target_avg_update[name] += delta * (
                    num_samples / self.total_samples
                )
