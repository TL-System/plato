...
class Trainer(basic.Trainer):
    ...
    def obtain_support_query_sets(self, config, trainset):
        """ Split the trainset to the support set and the query set """
        support_fraction = config["support_fraction"]
        trainset_size = len(trainset)
        support_set_size = int(trainset_size * support_fraction)
        query_set_size = trainset_size - support_set_size

        support_dataset, query_dataset = torch.utils.data.random_split(
            trainset, [support_set_size, query_set_size])
        ...
        try:
            ...
            if callable(custom_train):
                ...
                if callable(_train_loader):
                    ...
                    support_dataset, query_dataset = self.obtain_support_query_sets(
                        config, trainset)
