...
from plato.utils.rlfl.policies import td3
...
class RLAgent(simple_rl_agent.RLAgent):
    ...
    def __init__(self, config):
        ...
        # self.policy = ddpg.Policy(self.config.n_states, self.config.n_actions,
    ...
    async def prep_action(self):
        ...
        # self.action = (action + np.random.normal(
        #     0, self.config.exploration_noise,
        #     size=self.action_space.shape[0])).clip(self.action_space.low,
    ...
    def update_policy(self):
        ...
        if len(self.policy.replay_buffer) > self.config.batch_size:
            ...
            # SAC
            # critic_1_loss, critic_2_loss, policy_loss, ent_loss, alpha = self.policy.update(
            # )
        ...
        self.policy.writer.add_scalar('train/reward', self.episode_reward,
                                      self.current_episode)
        self.policy.writer.add_scalar('train/acc', mean(self.pre_acc),
                                      self.current_episode)
        self.policy.writer.add_scalar('train/ep_len', self.current_step,
                                      self.current_episode)
