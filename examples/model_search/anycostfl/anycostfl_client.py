...
class AnyCostFLLifecycleStrategy(DefaultLifecycleStrategy):
    """Lifecycle strategy that configures client models from server-provided rates."""

    _STATE_KEY = "anycostfl"

    @staticmethod
    def _state(context):
        return context.state.setdefault(AnyCostFLLifecycleStrategy._STATE_KEY, {})
...
def create_client(
    *,
    ...
    """Build an AnyCostFL client configured with the custom lifecycle strategy."""
    ...
    client._configure_composable(
        lifecycle_strategy=AnyCostFLLifecycleStrategy(),
