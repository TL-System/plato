...
def plot(clients, exp_name, culsum_plot=None, culsum_min=None, culsum_max=None):
    ...
    f, (ax1, ax2, ax3) = plt.subplots(nrows=num_sub_plots, ncols=1, figsize=(20,15))
    ...
    for i in range(len(culsum_plot.columns)):
        #resets which client
        if which_client == len(clients):
            which_client = 0
       
        #new plot index
        if i % len(clients) == 0 and i != 0:
            which_plot += 1
            which_exp += 1
            culsum_order = 0
        ...
        sns.lineplot(data=rewards, linewidth=2.5, legend=0, label=clients[which_client], marker=marker, markersize=10, linestyle=line, ax=list_of_subplots[which_plot])

        list_of_subplots[which_plot].set_title(exp_name[which_exp])
        which_client += 1   

        list_of_subplots[which_plot].set_ylabel("Cumulative average reward")
        list_of_subplots[which_plot].set_ylim(-500,1000) #Change this to wanted range
    ...
    f.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 0, 0., 0.), borderaxespad=0.1, ncol=len(clients), fontsize=30)

    plt.tight_layout() 

    pdf_name = "OneLegendClients"
...
if __name__ == '__main__':
    ...
    experiments = ["6out6_fedavg", "12out12_fedavg", "18out18_fedavg", "6out6_fedadp", "12out12_fedadp", "18out18_fedadp",
    "6out6_critic_grad", "12out12_critic_grad", "18out18_critic_grad"]
    target_files = ["CULSUM_AVG_6out6_fedavg", "CULSUM_AVG_12out12_fedavg","CULSUM_AVG_18out18_fedavg", 
    "CULSUM_AVG_6out6_fedadp", "CULSUM_AVG_12out12_fedadp", "CULSUM_AVG_18out18_fedadp",
    "CULSUM_AVG_6out6_critic_grad", "CULSUM_AVG_12out12_critic_grad", "CULSUM_AVG_18out18_critic_grad"]
    exp_names = ["FedAvg", "FedAdp", "Cascade"]
