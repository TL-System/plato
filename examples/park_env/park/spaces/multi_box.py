...
class MultiBox(core.Space):
    """
    Multiple boxes in m x R^n, where m can be varying
    I.e., each coordinate of each m is bounded.
    Example usage:
    node_features = spaces.MultiBox(low=np.array([0, 0], high=np.array([4, 5])))
    where each node in graph has two features, but the number
    of the nodes can be unbounded (could be 0 too)
    """
