...
def train(memory, q_val):
    ...
    for i, (_, _, reward, done) in enumerate(memory.reversed()):
        ...
        q_vals[
            len(memory) - 1 - i
        ] = q_val  # store values from the end to the beginning
    ...
    entropy_coef = max(
        (ENTROPY_RATIO - (episode_num / max_steps) * ENTROPY_DECAY), ENTROPY_MIN
    )
    actor_loss = (
...
while episode_num < MAX_EPISODES:
    ...
    if episode_num % 50 == 0:
        evaluate_policy()
        torch.save(
            actor.state_dict(), model_path + "/actor_model_seed_%s.pth" % str(SEED)
        )
        torch.save(
            critic.state_dict(), model_path + "/critic_model_seed_%s.pth" % str(SEED)
        )
