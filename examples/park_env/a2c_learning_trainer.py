...
class Trainer(basic.Trainer):
    def __init__(self, model=None):
        ...
        # Create environement and its name
        ...
        # Set actor and critic models and optimizers
        ...
        # Collects average rewards and a check to see if it's the first round or not
        self.server_reward = []
        self.avg_reward = []
        ...
        # Creates result folder if doens't exist
        if not os.path.exists(
            f"{Config().results.results_dir}_seed_{Config().server.random_seed}"
        ):
            os.makedirs(
                f"{Config().results.results_dir}_seed_{Config().server.random_seed}",
                exist_ok=True,
        ...
        # Creates seed folder if doesn't exist, if exists, remove and create new
        if not os.path.exists(
            f"{Config().results.seed_random_path}_seed_{Config().server.random_seed}"
        ):
            os.makedirs(
                f"{Config().results.seed_random_path}_seed_{Config().server.random_seed}",
                exist_ok=True,
    ...
    # Creates a tensor from numpy array
    def t(self, x):
    ...
    def train_model(self, config, trainset, sampler, cut_layer):
        ...
        actor_fisher_path = (
            f"{model_path}/{env_algorithm}{actor}{model_seed_path}{client_id}.pth"
        )
        critic_fisher_path = (
            f"{model_path}/{env_algorithm}{critic}{model_seed_path}{client_id}.pth"
        )
        ...
        # Load Omega
        if (
            self.episode_num >= Config().algorithm.max_round_episodes
            and Config().trainer.penalize_omega
        ):
            self.load_omega()
        ...
        while round_episodes < Config().algorithm.max_round_episodes:
            ...
            self.trace_idx = (
                (self.client_id - 1) % Config().algorithm.difficulty_levels
            ) * Config().algorithm.traces_per_task
            # need to add a random # to self.trace_idx a number between 0 and Config().algorithm.traces_per_task
            which_trace = (
                rand_trace[self.episode_num % (len(rand_trace))]
                if Config().algorithm.traces_per_task > 1
                else 0
            )
            state = self.env.reset(trace_idx=self.trace_idx + which_trace, test=True)
            ...
            while not self.done:
                ...
                next_state, reward, self.done, info = self.env.step(
                ...
                self.memory.add(
                    self.dist(probs=probs).log_prob(action),
                    self.critic(self.t(state)),
                    reward,
                    self.done,
                )
                ...
                if self.done or (self.steps % Config().algorithm.batch_size == 0):
                    ...
                    self.estimate_fisher(
                        self.train_helper(self.memory, last_q_val, fisher=True)
                    )
                    self.sum_fisher_diagonals()
                    # Accumulate fishers
                    if self.updates > 1:  # or self.episode_num > 1:
                        for n, _ in self.critic.named_parameters():
                            self.fisher_critic[n] = (
                                self.fisher_critic[n]
                                + fisher_critic_old[n] * self.updates
                            ) / (self.updates + 1)
                        for n, _ in self.actor.named_parameters():
                            self.fisher_actor[n] = (
                                self.fisher_actor[n]
                                + fisher_actor_old[n] * self.updates
                            ) / (self.updates + 1)
                    ...
                    self.save_metric(
                        actor_fisher_sum_path,
                        [self.actor_fisher_sum.tolist()],
                        first=(self.episode_num == 0)
                        and (self.steps == Config().algorithm.batch_size),
                    )
                    self.save_metric(
                        critic_fisher_sum_path,
                        [self.critic_fisher_sum.tolist()],
                        first=(self.episode_num == 0)
                        and (self.steps == Config().algorithm.batch_size),
                    )

                    critic_loss, actor_loss, entropy_loss = self.train_helper(
            ...
            round_episodes += 1
            print(
                "Episode number: %d, Reward: %d" % (self.episode_num, self.total_reward)
            )
    ...
    def train_helper(self, memory, q_val, fisher=False):
        ...
        entropy_loss = (
            torch.stack(memory.log_probs) * torch.exp(torch.stack(memory.log_probs))
        ).mean()
        entropy_coef = max(
            (
                Config().algorithm.entropy_ratio
                - (self.episode_num / Config().algorithm.batch_size)
                * Config().algorithm.entropy_decay
            ),
            Config().algorithm.entropy_min,
        )
        actor_loss = (
            -torch.stack(memory.log_probs) * advantage.detach()
        ).mean() + entropy_loss * entropy_coef

        if (
            Config().trainer.penalize_omega
            and self.episode_num >= Config().algorithm.max_round_episodes
        ):
    ...
    def sum_fisher_diagonals(self):
        ...
        self.critic_fisher_sum, self.actor_fisher_sum = (
            critic_fisher_sum,
            actor_fisher_sum,
        )
    ...
    def criterion_reg(self):
        ...
        for (name, param), (_, param_old) in zip(
            self.actor.named_parameters(), self.actor_old.named_parameters()
        ):
            loss_reg_actor += (
                torch.sum(self.omega_actor[name] * (param_old - param).pow(2)) / 2
            )
        for (name, param), (_, param_old) in zip(
            self.critic.named_parameters(), self.critic_old.named_parameters()
        ):
            loss_reg_critic += (
                torch.sum(self.omega_critic[name] * (param_old - param).pow(2)) / 2
            )
        return (
            Config().trainer.lamda * loss_reg_actor,
            Config().trainer.lamda * loss_reg_critic,
        )
    ...
    def load_fisher(self):
        ...
        actor_fisher_round, critic_fisher_round = np.zeros(GRAD_SIZE), np.zeros(
            GRAD_SIZE
        )
        ...
        self.save_metric(
            f'{path}{"_actor_fisher_grad"}',
            [actor_fisher_grad],
            first=self.episode_num <= Config().algorithm.max_round_episodes,
        )
        self.save_metric(
            f'{path}{"_critic_fisher_grad"}',
            [critic_fisher_grad],
            first=self.episode_num <= Config().algorithm.max_round_episodes,
        )

        # Set paths and names for saving
        actor, critic = "actor_fisher_matrix", "critic_fisher_matrix"
        model_path = Config().params["model_path"]
        client_id = str(self.client_id)
        env_algorithm = self.env_name + self.algorithm_name
    ...
    def load_model(self, filename=None, location=None):
        ...
        # Model path names & algorithm names
        ...
        if self.client_id == 0:
            logging.info(
                "[Server #%d] Loading models from %s, and %s.",
                os.getpid(),
                actor_model_path,
                critic_model_path,
            )
            ...
            logging.info(
                "[Client #%d] Loading a model from %s, and %s.",
                self.client_id,
                actor_model_path,
                critic_model_path,
            )
    ...
    def load_loss(self):
        """Loading previous round loss from a file"""
    ...
    def load_grads(self):
        """Loading previous round gradients from a file"""
        path = f"{Config().results.results_dir}_seed_{Config().server.random_seed}/{Config().results.file_name}_{self.client_id}"
        actor_grad = []
        critic_grad = []
    ...
    def save_metric(self, path, value, first=False):
        """Save whatever metric is passed in value to a file"""
    ...
    def save_model(self, filename=None, location=None):
        ...
        """Set model path & names"""
        actor_model_name = "actor_model"
        critic_model_name = "critic_model"
        env_algorithm = f"{self.env_name}{self.algorithm_name}"
        model_seed_path = f"_seed_{Config().server.random_seed}"

        # Save the loss if it's not the server loss
        if self.client_id != 0:
            path = f"{Config().results.results_dir}_seed_{Config().server.random_seed}/{Config().results.file_name}_{self.client_id}"
            self.save_loss(path)
        ...
        if self.client_id == 0:
            logging.info(
                "[Server #%d] Saving models to %s, and %s.",
                os.getpid(),
                actor_model_path,
                critic_model_path,
            )
            ...
            logging.info(
                "[Client #%d] Saving a model to %s, and %s.",
                self.client_id,
                actor_model_path,
                critic_model_path,
            )
    ...
    async def server_test(self, testset, sampler=None, **kwargs):
        ...
        # We use self.first_ittr_server since episode_num is reset to 0 when we are in this function
        ...
        for trace_idx in range(Config().algorithm.difficulty_levels):
            ...
            for epi in range(eval_episodes):
                ...
                state = self.env.reset(
                    trace_idx=trace_idx * Config().algorithm.traces_per_task + epi,
                    test=True,
                )
                state = self.obs_normalizer.normalize(state)
                steps = 0
            ...
            print("------------------")
            print(
                "Average Reward for client %s over trace %s for %s steps is %s"
                % (str(self.client_id), str(trace_idx), str(steps), str(avg_reward))
            )
            print("------------------")
