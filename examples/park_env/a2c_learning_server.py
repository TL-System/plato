...
class A2CServer(fedavg.Server):
    ...
    async def federated_averaging(self, updates):
        ...
        if Config().server.percentile_aggregate:
            percentile = min(
                Config().server.percentile
                + Config().server.percentile_increase * self.current_round,
                100,
            )
            ...
            clients_selected_size = len(
                [i for i in metric_list if i <= metric_percentile]
            )
        ...
        for i, update in enumerate(weights_received):
            ...
            if not Config().server.percentile_aggregate:
                ...
                if metric <= metric_percentile:
                    ...
                    for name, delta in update_from_actor.items():
                        actor_avg_update[name] += (
                            delta
                            * 1.0
                            / clients_selected_size
                            * (
                                norm_fisher_actor[name]
                                if Config().server.mul_fisher
                                else 1.0
                            )
                        )
                    for name, delta in update_from_critic.items():
                        critic_avg_update[name] += (
                            delta
                            * 1.0
                            / clients_selected_size
                            * (
                                norm_fisher_critic[name]
                                if Config().server.mul_fisher
                                else 1.0
                            )
                        )
