...
class Trainer(pers_basic.Trainer):
    ...
    def perform_test_op(
        ...
        for _, (examples, labels) in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                features = self.personalized_model.encoder(examples)
                preds = self.personalized_model.clf_fc(features).argmax(dim=1)
    ...
    def pers_train_one_epoch(
        ...
        for _, (examples, labels) in enumerate(local_progress):
            ...
            features = self.personalized_model.encoder(examples)
            preds = self.personalized_model.clf_fc(features)
        ...
        if (epoch - 1) % epoch_log_interval == 0 or epoch == pers_epochs:
            ...
            test_outputs = self.perform_test_op(test_loader)
