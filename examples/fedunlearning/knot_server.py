...
class Server(fedunlearning_server.Server):
    ...
    async def wrap_up_processing_reports(self):
        ...
        if hasattr(Config().server, "do_optimized_clustering") and Config(
        ).server.do_optimized_clustering and self.current_round == 2:
        ...
        for cluster_id in range(self.num_clusters):
            if (self.current_round
                    == data_deletion_round) and not self.clustered_retraining[
                        cluster_id] and not self.initialize_optimization:
                for client_id, init_round in self.round_first_selected.items():
                    if client_id in clients_to_delete and self.clusters[
                        ...
                        self.rollback_round[
                            cluster_id] = self.earliest_round[cluster_id] - 1
                        logging.info(
                            fonts.colourize(
    ...
    async def customize_server_response(self, server_response, client_id=None):
        ...
        if cluster_id in self.clustered_retraining and self.clustered_retraining[
                cluster_id]:
    ...
    def save_to_checkpoint(self):
        ...
        for cluster_id in range(self.num_clusters):
            ...
            logging.info(
                "[%s] Saving the checkpoint for cluster #%s to %s/%s.", self,
                cluster_id, checkpoint_path, filename)
    ...
    def init_cluster_states(self):
        ...
        if hasattr(Config().server, "do_optimized_clustering"):
            self.initialize_optimization = Config(
            ).server.do_optimized_clustering
        else:
    ...
    def did_stablize(self):
        ...
        if hasattr(Config().trainer, "target_accuracy"):
            ...
            if len(self.recent_global_accuracies) >= self.recent_history_size:
                return global_acc > target_accuracy and standard_deviation < target_accuracy_std
            else:
                return False
        ...
        if hasattr(Config().trainer, "target_perplexity"):
            ...
            if len(self.recent_global_accuracies) >= self.recent_history_size:
                return 0 < global_acc < target_perplexity and standard_deviation < target_perplexity_std
            else:
                return False
        ...
        if self.current_round == 1 and (
            ...
            for client_update in updates:
                ...
                if self.clustered_retraining[self.clusters[client_id]] is True:
                    if abs(staleness) <= self.current_round - Config(
                    ).clients.data_deletion_round - 1:
                        if self.clusters[client_id] in self.clustered_updates:
                            self.clustered_updates[
                                self.clusters[client_id]].append(client_update)
                        else:
                            self.clustered_updates[
                                self.clusters[client_id]] = [client_update]
    ...
    async def process_reports(self):
        ...
        if hasattr(Config().server, "do_clustered_test") and Config(
        ).server.do_clustered_test and not self.initialize_optimization:
            # Testing the updated clustered model directly at the server

            # First, obtain the set of cluster IDs that have been aggregated
            updated_cluster_ids = {
                self.clusters[client_id]
                for (client_id, __, __, __) in self.updates
            }
        ...
        if hasattr(Config().server, 'do_test') and Config().server.do_test:
            ...
            self.trainer.model.load_state_dict(self.aggregate_models(),
                                               strict=True)
    ...
    async def wrap_up(self):
        ...
        if self.did_stablize(
        ) or self.current_round >= Config().trainer.rounds and max(
                self.clustered_retraining.values()):
            logging.info(
                fonts.colourize(
            ...
            if target_accuracy and self.accuracy >= target_accuracy:
                logging.info(
                    "[%s] Target accuracy and standard deviation reached.",
                    self)
                await self.close()
    ...
    async def select_clients(self, for_next_batch=False):
        ...
        if hasattr(Config().server, "do_optimized_clustering") and Config(
        ).server.do_optimized_clustering and not self.initialize_optimization:
    ...
    def cosine_similarity(self, updates):
        ...
        for i, update in enumerate(deltas_received):
            ...
            similarity = (
                F.cosine_similarity(current - initial, deltas, dim=0) + 1) / 2
    ...
    def convert_to_solver(self, client_training_times):
        ...
        similarity_interval = (max(similarity_list) -
                               min(similarity_list)) / self.num_clusters

        training_time_interval = (max(training_time_list) -
                                  min(training_time_list)) / self.num_clusters
        ...
        for cluster_id in range(self.num_clusters):
            training_anchor_distances.append([
                abs(training_time_interval * cluster_id +
                    min(training_time_list) - time)
                for time in training_time_list
            ])
            similarity_anchor_distances.append([
                abs(similarity_interval * cluster_id + min(similarity_list) -
                    similarity) for similarity in similarity_list
            ])
