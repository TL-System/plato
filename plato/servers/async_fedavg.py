...
from plato.servers import async_base as base
...
class Server(base.Server):
    ...
    def __init__(self, model=None, algorithm=None, trainer=None):
        ...
        logging.info(
            "[Server #%d] Started training on %s "
            "with at most %s clients issued per asynchronous step.",
            os.getpid(), self.total_clients, self.max_clients_per_step)

        # starting time of a global training
    ...
    def configure(self):
        ...
        if target_accuracy:
            logging.info("Training: %s steps or %s%% accuracy\n",
                         self.step_limit, 100 * target_accuracy)
        else:
