...
from plato.servers.async_timer import timer_run
...
class ServerEvents(socketio.AsyncNamespace):
    ...
    async def on_client_payload_done(self, sid, data):
        ...
        await self.plato_server.client_payload_done(sid, data['id'],
                                                    data['obkey'])
    async def on_timer_alive(self, sid, data):
        """ The timer sends a heartbeat. """
        await self.plato_server.start_timer(sid)

    async def on_start_step(self, sid, data):
        """ The timer indicates the start of a new asynchronous step. """
        await self.plato_server.start_async_step(sid)
...
class Server:
    ...
    async def select_clients(self):
        ...
        logging.info("[Server #%d] Start to select clients at Step %s/%s.", os.getpid(),
                     self.current_step,
                     self.total_steps)
    ...
    async def send(self, sid, payload, client_id) -> None:
        ...
        if self.s3_client is not None:
            payload_key = f'server_payload_{os.getpid()}_{self.current_step}'
    ...
    def create_timer(self):
        """ Create a server-side timer. """
        proc = mp.Process(target=timer_run, args=())
        proc.start()

    async def start_timer(self, sid):
        logging.info("[Server #%d] The timer arrived.", os.getpid())
        await self.sio.emit('start_timer', {}, room=sid)

    async def start_async_step(self, sid):
        logging.info("[Server #%d] Step %s/%s triggered by the timer.",
        ...
        logging.info("[Server #%d] Step %s/%s ended.",
                     os.getpid(), self.current_step, self.total_steps)
        self.current_step += 1
