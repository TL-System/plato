...
def get_lr_schedule(
    ...
    for scheduler in lr_schedule:
        if scheduler == "CosineAnnealingLR":
            ...
            factor = (
                Config().trainer.lr_factor
                if hasattr(Config().trainer, "lr_factor")
                else 0.1
            )
            patience = (
                Config().trainer.lr_patience
            ...
            start_factor = (
                Config().trainer.lr_start_factor
                if hasattr(Config().trainer, "lr_start_factor")
                else 1.0 / 3.0
            )
            end_factor = (
                Config().trainer.lr_end_factor
            ...
            base_lr = (
                Config().trainer.lr_base
                if hasattr(Config().trainer, "lr_base")
                else 0.01
            )
            max_lr = (
                Config().trainer.lr_max if hasattr(Config().trainer, "lr_max") else 0.1
            )
            # Step size is the number of training iterations
            step_size_up = (
                Config().trainer.lr_step_up
                if hasattr(Config().trainer, "lr_step_up")
                else 2000
            )
            step_size_down = (
                Config().trainer.lr_step_down
            ...
            num_iters = (
                Config().trainer.num_iters
                if hasattr(Config().trainer, "num_iters")
                else 50
            )
            returned_schedules.append(
                optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, num_iters)
            )
        else:
