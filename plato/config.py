...
class Config:
    ...
    def get_client_pool_for_edge() -> int:
        ...
        clients_pool = list(
            range(starting_client_id + 1, starting_client_id + 1 + edge_total_clients)
        )
        return clients_pool
    ...
    def get_edge_id_for_client() -> int:
        ...
        for i, count in enumerate(edges_launched_clients):
            ...
            if client_id <= total:
                return i + 1
        raise ValueError(
            f"No edge server found for the client: client_id: {client_id}, total number of clients: {total}"
        )
