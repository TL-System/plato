...
def plot_edge_agg_num_vs_accuracy(accuracy_list, edge_agg_num_list,
                                  figure_path):
    """
    Plot a figure showing the number of aggregation rounds on edge servers
    vs. glboal model accuracy.
    """
    x_value = [
        sum(edge_agg_num_list[:i + 1]) for i in range(len(edge_agg_num_list))
    ]
    y_value = accuracy_list
    x_label = 'Aggregation rounds on edge servers'
    y_label = 'Accuracy (%)'
    figure_name = 'edge_agg_num_vs_accuracy'
    plot(x_value, x_label, y_value, y_label, figure_path, figure_name)
