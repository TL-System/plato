...
def get_lr_schedule(optimizer: optim.Optimizer, iterations_per_epoch: int):
    ...
    if Config().training.lr_gamma != 0.0 and Config(
        ...
        lambdas.append(lambda it: Config().training.lr_gamma**bisect.bisect(
            milestones, it))

    # Add linear learning rate warmup if specified
    if Config().training.lr_warmup_steps != '':
        warmup_iters = Step.from_str(Config().training.lr_warmup_steps,
