...
!!! note "Running on macOS"
    ...
    !!! tip "Required System Libraries"
        The Polaris example depends on `cvxopt`, which requires several system-level libraries for compilation:
    ...
    !!! tip "Potential issue cvxopt Build Failure - Missing umfpack.h"
        ...
        **Solution:**
        This happens when `cvxopt` cannot find the UMFPACK headers from SuiteSparse. Set the proper environment variables before running:
