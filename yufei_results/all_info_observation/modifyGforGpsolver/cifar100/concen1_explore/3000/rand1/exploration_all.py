    ...
    with open(selection_file_name) as f:
        ...
        for line in f.readlines():
            ...
            for temp in all_selection[20:]:
                ...
                if counter % 10 == 0:
                    exploration_rate.append(len(num_list) / 2.0)
...
plt.plot(x_value["Polaris"][:141], y_value["Polaris"][:141], label="Polaris")
plt.plot(
    x_value["Pisces"][:107], y_value["Pisces"][:107], label="Pisces", linestyle="--"
)
plt.plot(x_value["Oort"][:69], y_value["Oort"][:69], label="Oort", linestyle=":")
plt.plot(
    x_value["FedBuff"][:82], y_value["FedBuff"][:64], label="FedBuff", linestyle="-."
)
