...
filename_temp = "./oort.csv"
df_temp = pd.read_csv(filename_temp)

x_temp = df_temp["elapsed_time"]
print(len(exploration_rate))

df_explore = pd.DataFrame({'Elapsed time (s)': x_temp, 'Exploration rate (%)': exploration_rate})#[x_temp, exploration_rate],columns=["Elapsed time (s)", "Exploration rate (%)"])#.transpose()
...
figure_file_name = "exploration_rate.pdf"
